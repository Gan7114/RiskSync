...
var makeEvmClient = (runtime2) => {
  const { chainSelectorName } = runtime2.config.evm;
  ...
  if (!network248)
    throw new Error(`Network ${chainSelectorName} not found`);
  return new ClientCapability(network248.chainSelector.selector);
};
var callView = (runtime2, contract, data) => makeEvmClient(runtime2).callContract(runtime2, {
  call: encodeCallMsg({
    from: zeroAddress,
    to: contract,
    data
...
var readEnabledAssets = (runtime2) => {
  const callData = encodeFunctionData({
    abi: ASSET_REGISTRY_ABI,
    functionName: "getEnabledAssets",
    args: []
  });
  const result = callView(runtime2, runtime2.config.evm.assetRegistryAddress, callData);
...
var readRouterState = (runtime2, asset) => {
  const callData = encodeFunctionData({
    abi: MULTI_ASSET_ROUTER_ABI,
    functionName: "assetRiskState",
    args: [asset]
  });
  const result = callView(runtime2, runtime2.config.evm.multiAssetRouterAddress, callData);
  const decoded = decodeFunctionResult({
    abi: MULTI_ASSET_ROUTER_ABI,
    functionName: "assetRiskState",
    data: bytesToHex(result.data)
...
var readCircuitBreakerLevel = (runtime2) => {
  const callData = encodeFunctionData({
    abi: CIRCUIT_BREAKER_ABI,
    functionName: "currentLevel",
    args: []
  });
  const result = callView(runtime2, runtime2.config.evm.circuitBreakerAddress, callData);
  const decoded = decodeFunctionResult({
    abi: CIRCUIT_BREAKER_ABI,
    functionName: "currentLevel",
    data: bytesToHex(result.data)
...
var estimateCcipFee = (runtime2) => {
  const destinationSelector = BigInt(runtime2.config.evm.ccipDestinationSelector);
  const callData = encodeFunctionData({
    abi: CCRB_ABI,
    functionName: "estimateFee",
    args: [destinationSelector]
  });
  const result = callView(runtime2, runtime2.config.evm.ccrbAddress, callData);
  const decoded = decodeFunctionResult({
    abi: CCRB_ABI,
    functionName: "estimateFee",
    data: bytesToHex(result.data)
...
var getOffchainVolatility = (sendRequester, request) => {
...
var assetKey = (address) => address.toLowerCase();
var shortAddress = (address) => `${address.slice(0, 6)}...${address.slice(-4)}`;
var classifySeverity = (onchainScore, offchainVolatilityPct, config) => {
  const scoreSeverity = onchainScore >= config.thresholds.emergencyScore ? 3 : onchainScore >= config.thresholds.warningScore ? 2 : onchainScore >= Math.max(1, config.thresholds.warningScore - 20) ? 1 : 0;
  const volSeverity = offchainVolatilityPct >= config.thresholds.volatilityPct * 2 ? 2 : offchainVolatilityPct >= config.thresholds.volatilityPct ? 1 : 0;
  if (scoreSeverity >= 2 && volSeverity >= 1)
    return 3;
  return Math.max(scoreSeverity, volSeverity);
};
var actionForSeverity = (severity) => {
  if (severity >= 3)
    return "broadcast_cross_chain_and_harden_ltv";
  if (severity === 2)
    return "tighten_ltv_and_raise_alert";
  if (severity === 1)
    return "monitor_closely";
  return "none";
};
var onTrigger = async (runtime2) => {
  runtime2.log("risk-orchestrator: start");
  const enabledAssets = readEnabledAssets(runtime2);
  runtime2.log(`enabled assets discovered: ${enabledAssets.length}`);
  const metadataByAsset = new Map(runtime2.config.assets.map((asset) => [assetKey(asset.address), asset]));
  const httpClient = new ClientCapability2;
  const decisions = [];
  for (const asset of enabledAssets) {
    const state = readRouterState(runtime2, asset);
    const meta = metadataByAsset.get(assetKey(asset));
    let offchainVolatilityPct = 0;
    if (meta?.coingeckoId) {
      try {
        offchainVolatilityPct = await httpClient.sendRequest(runtime2, getOffchainVolatility, consensusMedianAggregation())({
          priceUrlTemplate: runtime2.config.api.priceUrlTemplate,
          coingeckoId: meta.coingeckoId
        }).result();
      } catch (error) {
        runtime2.log(`offchain volatility fetch failed for ${meta.symbol}: ${String(error)}`);
      }
    }
    const severity = classifySeverity(state.score, offchainVolatilityPct, runtime2.config);
    const decision = {
      asset,
    ...
    runtime2.log(`[${decision.symbol}] score=${decision.onchainScore} tier=${decision.onchainTier} ` + `vol24h=${decision.offchainVolatilityPct.toFixed(2)}% severity=${decision.severity}`);
  }
  decisions.sort((a, b) => {
  ...
  let breakerLevel = 0;
  try {
    breakerLevel = readCircuitBreakerLevel(runtime2);
  } catch (error) {
    runtime2.log(`could not read circuit breaker level: ${String(error)}`);
  }
  let estimatedCcipFeeWei = "0";
  if (globalSeverity >= 2) {
    try {
      estimatedCcipFeeWei = estimateCcipFee(runtime2);
      runtime2.log(`ccip fee estimate for alert path: ${estimatedCcipFeeWei} wei`);
    } catch (error) {
      runtime2.log(`ccip fee estimate failed: ${String(error)}`);
    }
  }
  const output = {
  ...
  runtime2.log(`global severity=${output.globalSeverity} action=${output.recommendedAction} ` + `enabled_assets=${output.enabledAssetCount}`);
  return output;
};
async function main() {
